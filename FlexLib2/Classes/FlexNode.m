               ...
               RootView:(FlexRootView*)rootView
{
    ...
    [view configureLayoutWithBlock:^(YGLayout* layout){
        ...
        for (FlexAttr* attr in layoutParam) {
            if([attr.name compare:@"style" options:NSLiteralSearch]==NSOrderedSame){
...
+(FlexNode*)buildNodeWithXml:(GDataXMLElement*)element
{
    ...
    NSMutableArray<FlexAttr *> *viewAttrs = [NSMutableArray array];
    [[element attributes] enumerateObjectsUsingBlock:^(GDataXMLNode *  _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        NSString *name = obj.name;
        NSCharacterSet* whiteSet = [NSCharacterSet whitespaceAndNewlineCharacterSet] ;
        NSArray *components = [name componentsSeparatedByString:@":"];
        NSAssert(2 == [components count], @"xml error.");
        NSString *namespace = [components[0] stringByTrimmingCharactersInSet:whiteSet];
        NSString *attrName = iOSFlexKeyFromAndroid([components[1] stringByTrimmingCharactersInSet:whiteSet]);
        NSString *attrValue = [[obj stringValue] stringByTrimmingCharactersInSet:whiteSet];
        BOOL isLayout = [namespace isEqualToString:@"app"];
        BOOL isAttr = [namespace isEqualToString:@"ios"];
        if (isLayout || isAttr) {
            FlexAttr* attr = [[FlexAttr alloc] init];
            attr.name = attrName;
            if (isLayout) {
                ...
                else if ([attrName isEqualToString:@"onPress"]) {
                    node.onPress = attrValue;
                }
                else {
